...
宿主提供的是已构建的客户端 bundle，因此启动前 `pnpm run build` 必须已产出每个 `lib/client.js`；缺失的 bundle，或比所在包源码更旧的 bundle，会以一条构建说明加包／路径列表的方式让激活大声失败。过期比较只在激活时运行，稳态 reconcile 不参与，因此开发 watch 进程可以在不打断活跃会话的情况下完成重建；不带源码树的包也永不会被报为过期。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出。本包自身不接受任何插件配置。
