...
| `MarkdownText`、`MarkdownDelegateProvider`、`CodeBlock`、`DiagramFence` | 不可信 GFM 与 TeX 数学、owner 委托的 HTTP(S) 导航、高亮代码，以及定稿的 mermaid 图表。`CodeBlock` 可通过 `lineNumbers` 开启行号；复制的源码不含行号栏，`contentRef` 则向需要把稳定源码包装节点用作滚动区的 owner 提供该节点。调用方提供自己的语言与复制工具栏时，设置 `showHeader={false}`。 |
