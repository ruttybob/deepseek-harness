...
| `MarkdownText`, `CodeBlock`, `DiagramFence` | Untrusted GFM with TeX math, highlighted code, and settled mermaid diagrams. `CodeBlock` accepts opt-in `lineNumbers`; copied source excludes the gutter, and `contentRef` exposes its stable source wrapper to an owner that uses it as a scrollport. |
