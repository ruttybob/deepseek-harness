...
| `MarkdownText`, `MarkdownDelegateProvider`, `CodeBlock`, `DiagramFence` | Untrusted GFM with TeX math, owner-delegated HTTP(S) navigation, highlighted code, and settled mermaid diagrams. `CodeBlock` accepts opt-in `lineNumbers`; copied source excludes the gutter, and `contentRef` exposes its stable source wrapper to an owner that uses it as a scrollport. Set `showHeader={false}` when the owner supplies its own language and copy toolbar. |
