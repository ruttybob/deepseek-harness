...
Session open 只会把结果落进快照，从不 reject：Host 标记的失败、本地打开故障，或在打开 deadline（`SessionOptions.openTimeoutMs`，默认 15 秒，与 connection generation-ready 同一量级）内仍未落地的 snapshot，都会把 `openState` 落到 `'error'` 并携带 `openError`，而不是滞留在 `'loading'`；只要状态不是 `'open'`，下一次 open 就会重新进入。打开 Assistant baseline 中未通过无损校验的 member 会让 open 降级为仅持久条目（记录日志，活跃 attempt 保持采纳），而不是让 open 失败。
