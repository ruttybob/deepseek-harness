...
The Session open settles into the snapshot and never rejects: a Host-marked failure, a local opening fault, or a snapshot that has not landed within the opening deadline (`SessionOptions.openTimeoutMs`, default 15 s — the connection generation-ready scale) lands `openState` in `'error'` with `openError` instead of latching `'loading'`, and the next open re-enters while the state is not `'open'`. An opening Assistant baseline member that fails lossless validation degrades the open to the durable entries alone (logged, live attempt retained) instead of failing it.
